...
2. Install the package by calling `colcon build --symlink-install --packages-select rover_description`
3. (Re-)Source environment `source install/setup.bash`
4. Launch description test:
   ```
   ros2 launch rover_description test_rover_description.launch.py
   ```
