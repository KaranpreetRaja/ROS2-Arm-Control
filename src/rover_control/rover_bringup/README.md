...
1. If you want to test hardware with `ForwardCommandController` first load and configure it. Controller types are e.g., "position", "velocity", and depend on configured names in the [`config/rover_controllers.yaml`](config/rover_controllers.yaml):
   ```
...
3. Send command to the controller, either:
   ...
   ```
   ros2 launch rover_bringup test_forward_position_controller.launch.py
   ```
